...
class MasterNode(Node):
    ...
    def blind_insert(self, base_pose, layer_index, yaw_offset=0.0):
        ...
        # 3. Z (베이스 높이에서 층수만큼 블록 높이를 계산해 위로 쌓음)
    ...
    def build_carrot(self):
        self.get_logger().info("🥕 [당근] 노란색(Pick) -> 초록색(Base) -> 노란색(Pick)")
        if self.pick_target("2x2_yellow"):
            self.call(self.cli_h, Trigger.Request())
            base_pose = self.find_target_with_retry("2x2_green") # 1층 베이스 스캔
            if base_pose:
                self.blind_insert(base_pose, layer_index=1)
                self.call(self.cli_h, Trigger.Request())
                ...
                if self.pick_target("2x2_yellow"):
                    self.call(self.cli_h, Trigger.Request())
                    self.blind_insert(base_pose, layer_index=2) # 스캔 없이 바로 꽂음
    ...
    def build_traffic_light(self):
        ...
        if self.pick_target("2x2_yellow"):
            self.call(self.cli_h, Trigger.Request())
            base_pose = self.find_target_with_retry("2x2_green")
            if base_pose:
                self.blind_insert(base_pose, layer_index=1)
                self.call(self.cli_h, Trigger.Request())
    ...
    def build_hammer(self):
        ...
        # 💡 망치의 경우 빨간색이 2개 필요함.
        # 하나를 먼저 집어들면, 바닥에 남은 다른 빨간색이 베이스가 됨! (충돌 X)
        if self.pick_target("2x2_red"):
            self.call(self.cli_h, Trigger.Request())
            base_pose = self.find_target_with_retry("2x2_red")
            if base_pose:
    ...
    # =========================================================
    def run(self):
        self.get_logger().info("🚀 STARTING BLIND-STACK ASSEMBLY SEQUENCE")
        self.call(self.cli_h, Trigger.Request())
        self.call(self.cli_g, SetBool.Request(data=False))
        time.sleep(1.0) 
